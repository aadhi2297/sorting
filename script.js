(function () {
  ...
  generateBtn.addEventListener("click", () => {
    if (!isSorting) {
      ...
      showMessage(
        "A sort is currently in progress. Please wait for it to finish."
      );
    }
  });
  ...
  startBtn.addEventListener("click", () => {
    if (isSorting) {
      showMessage("A sort is already running.");
      return;
    }
    selectedAlgorithm = algorithmSelector.value;
    if (array.length === 0) {
      showMessage("Please generate an array first.");
      return;
    }
  ...
  arraySizeRange.addEventListener("input", (e) => {
    ...
    if (!isSorting && customArrayInput.value === "") {
      handleGenerateArray();
    }
  });
  customArrayInput.addEventListener("input", () => {
    ...
    generateBtn.textContent = isCustomInput
      ? "Create Custom Array"
      : "Generate New Array";
  });
  ...
  const sleep = (ms) =>
    new Promise((resolve, reject) => {
      const checkStop = () => {
        if (isStopped) {
          reject(new Error("Sort stopped by user"));
        } else {
  ...
  function handleGenerateArray() {
    ...
    if (customInput !== "") {
      ...
      if (validValues.length > 200) {
        showMessage(
          "The maximum number of elements is 200. Please reduce your list size."
        );
        return;
      }
      ...
      for (let i = 0; i < currentArraySize; i++) {
        newArray.push(
          Math.floor(Math.random() * (maxBarHeight - minBarHeight)) +
            minBarHeight
        );
      }
      if (arrayType === "sorted") {
  ...
  function updateVisualization(indicesToHighlight = [], color = "#3b82f6") {
    ...
    for (let index of indicesToHighlight) {
      if (bars[index]) {
      ...
      for (let i = 0; i < array.length - 1; i++) {
        ...
        for (let j = i + 1; j < array.length; j++) {
          ...
          if (array[j] < array[minIndex]) {
            if (minIndex !== i) {
              visualizerContainer.children[minIndex].style.backgroundColor =
                "#3b82f6";
            }
            minIndex = j;
            visualizerContainer.children[minIndex].style.backgroundColor =
  ...
  async function mergeSort(left, mid, right) {
    ...
    for (let i = left; i <= right; i++) {
      bars[i].style.backgroundColor = "#8b5cf6";
    }
